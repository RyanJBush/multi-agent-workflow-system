    ...
    <div className="space-y-3">
      ...
      {data.gaps.length > 0 && (
        <div>
          ...
          <ul className="list-disc pl-5 space-y-1">
            {data.gaps.map((g, i) => (
              <li key={i} className="text-sm text-gray-500 italic">{g}</li>
            ))}
          </ul>
        </div>
      )}
...
function SummaryCard({ data }: { data: SummaryOutput }) {
  return (
    <p className="text-sm text-gray-700 leading-relaxed">
      {data.summary}{" "}
      <span className="text-xs text-gray-400">({data.word_count} words)</span>
    </p>
  );
}
...
export function AgentOutputCard({ result }: Props) {
  ...
  return (
    <div className={`rounded-xl border p-5 ${isSuccess ? "border-gray-200 bg-white" : "border-red-200 bg-red-50"}`}>
